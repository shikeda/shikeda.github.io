...
|F00762_01|F00762_01a|優 |  |  | イウ| | |  |

この例では、二つの区分した一方に、注文IDと同じIDを
もう一方にa番号を追加した。


音武（L-HV）「フ」「ム（H）」の場合は次のようになる。ここでは末尾にaとbを追加している。

|注文ID|音注ID| 単字_見出し | 声点 | 声点型 | 仮名注 | 仮名型 | 反切 | 類音 |
|-|-|--|-----|-----|-----|----|----|----|
|F00841_01|F00841_01|侮 | 平／上濁| 平／上| | | | 音武|
|F00841_01|F00841_01a|侮 |  |  | フ| | |  |
|F00841_01|F00841_01b|侮 |上 |上  | ム| | |  |

音注は三つの要素に区分される。二つ以上の音注IDが必要になった場合は、
順にa、b、cを付すこととする。
...
しかし、ここに示した9種のパターンを調査するのは手間がかかるので、
...
|2 |F01058_01|F01058_01|邏 | ＊去 |＊去  | | |盧賀反 |  |
|3 |F01361_01|F01361_01|避 |　 |　  | | |裨豉反 |  |
|3 |F01361_01|F01361_01a|豉 |　 |　  |シ | | |  |
|4 |F00016_03|F00016_03|佛 |＊入 |＊入 || | 符弗反|  |
|4 |F00016_03|F00016_03a|符 |　 |　  |フ|フ | |  |
|4 |F00016_03|F00016_03b|弗|　入 |　入  || | |  |

1の「辵」は反切「丑略反」だけなので問題ない。  
2の「邏」は反切「盧賀反」に加えて反切下字が「去」の声点であることを示す必要がある。
これはDHSJRの方針次第だが、反切上字と反切下字の声点を続けて記入してみた。  
3の「避」は反切「裨豉反」に加えて反切下字「豉」に仮名注「シ」があるものである。
前者は「単字_見出し」を「避」とし、後者は「単字_見出し」を「豉」とすることで対応させた。  
4の「佛」は反切、声点、仮名注が複合するものである。上に示したように三つに区分した。
...
しかし、これでは、「仁」の声点が「平濁」であり、同じ行に「ニン」を配置するのは
清濁に関わる整合性がとれない。

そこで3行に分ければ、掲出字（被注字）と、音注（仮名注、声点、類音）との対応は明確になる。
...
音注IDが複数必要な場合、注文IDにb, c, ..., nを追加することで行う。
