...
そこで、HDICプロジェクトしして、DHSJR の仕様に合わせたデータ公開を公開することとした。
...
- DHSJRプロジェクトは、平安・鎌倉期から現代までの漢字音・漢語音を、仮名注、声点、反切などの注記に即して資料横断的に検索可能とすることを目的とし、早稲田大学・加藤大鶴氏が代表を務める。
- DHSJRに名義抄を収録すべく、KRMの全文テキスト（`krm_main.tsv`や`krm_notes.tsv`）から音注を抽出して適合を図った。
- DHSJRが規定する23のデータ列（「単字＿見出し」「声点」「仮名注」「反切」「類音」「出現位置」など）と、KRMデータの各項目とのマッピングを個別に検討した。
- 2025年2月17日に加藤大鶴氏をはじめとするDHSJR関係者と協議を開始し、DHSJR側の確認・点検を経て2025年9月10日に公開した。資料IDは`30-048-02`、ファイル名は`30-048-02_RMK`である。
- 2026年6月23日から7月12日まで、DHSJR提供データの点検・整備を行った。作業の記録は[DHSJRリポジトリのコミット履歴](https://github.com/daikaku/DHSJR/commits/main/)を参照。

これらの詳細、および提供データ整備中に気付いた問題点は、事例研究「[DHSJRとの連携](/docs/krm/08-case-studies/08-05-dhsjr/)」にSupporting Documentationとしてまとめてある。KRMを漢字音資料として利用する際に参考となる説明を多く含むので、あわせて参照されたい。
