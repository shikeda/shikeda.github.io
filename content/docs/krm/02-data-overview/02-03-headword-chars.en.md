...
| entry_id             | The ID of the **`Entry`** (from `krm_main`) to which the **`Headword`** (containing this constituent character) belongs. This ID is a 5-digit numeric value starting with 'F'. For some newly added **`Entries`**, a 'b' suffix is appended. A small number of rows use '〔抹消〕' instead: these record a character that was cancelled (struck through) in the original manuscript and therefore has no corresponding **`Entry`** in `krm_main`/`krm_notes`. The row is kept, rather than deleted, so the character sequence on the page is not misread as missing data.      |
