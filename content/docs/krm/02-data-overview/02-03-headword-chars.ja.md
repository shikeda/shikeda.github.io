...
| entry_id               | この文字が属する掲出字（見出し語）の項目（krm_mainにおける項目）のID（Fで始まる5桁の数値）。一部、追加した掲出項目にはb番号を付す。ごく一部の行は「〔抹消〕」となる。これは原本上でその文字が抹消（見せ消ち）されており、krm_main/krm_notesに対応する項目が存在しないことを示す。行自体は削除せず残してある（削除すると頁上の文字連続が欠損データと誤認されるおそれがあるため）。      |
