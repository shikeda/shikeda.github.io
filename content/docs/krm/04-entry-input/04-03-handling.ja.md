...
掲出字に用いられる異体字は、原文通りの字形またはそれに近い字形で入力することに
...
熟語の掲出字に見える代用符号（substitution mark）「｜」は、
...
合符（compound mark, 連字符）はハイフンマイナス（-, U+002D）で表す。 例は僅少である。
...
割注双行（interlinear notes / small character definition in two lines, 雙行夾注）が基本なので右から左の順に記載する。
