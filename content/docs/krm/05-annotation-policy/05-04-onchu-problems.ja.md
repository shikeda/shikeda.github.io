...
- `kazama_location`: K01016330, `hanzi_entry`: 仱,  
    `definition`:
    1. 紺音, `remarks`: 広韻になし。「紺」は広韻「古暗切」（去声勘韻）。
    2. 渠廉反, `remarks`: 万象名義「渠廉反」。平声鹽韻羣母。
    3. 古樂人, `remarks`: 万象名義「古樂人也」。
- `kazama_location`: K01016340, `hanzi_entry`: 佡,   
    `definition`: 
    1. 俗仚字, `remarks`: 「仚」は説文「人在山上」。
    2. 丘豉反, `remarks`: 広韻になし。「仚」は広韻「許延切」（平声仙韻）。「丘豉反」は去声寘韻渓母。「企」は広韻「丘弭切」（上声紙韻）、「去智切」（去声寘韻）あり、後者の音に対応。
    3. 呼莚反, `remarks`: 龍龕手鏡「佡仚　許延反。輕也。二。」（巻1人部）。
- `kazama_location`: K01016410, `hanzi_entry`: ⿰亻企, 
    `definition`: 
    1. 俗企字


最後の `K01016410`「⿰亻企」はUnicodeのu2cf7e（𬽾）にあるが、
環境により表示されない。GlypWikiで表示すると
![⿰亻企](https://glyphwiki.org/glyph/u2cf7e.50px.png)となる。

さて、`K01016340`「佡」は広韻に見えない。万象名義や宋本玉篇にも見えない。
...
最初のK0101632「㑫」は玄応音義巻第四大灌頂經第一卷に
```
郁佡　丘豉反。經文作㑫、非也。
```
...
`K01016421`「伱／你」はその前の「佡」「⿰亻企」の類似字形である。

## 同音字注

同音字注（Homophone glosses）は類音注（Similar sound notes）ともいう。掲出字の音を、同音または類似音の漢字1字で示す注記で、「音◯」「◯音」の形式が大半を占める。二字熟語では「二音」を付して「◯△二音」の形式をとることもある。

例えば、「留」には「音流」とあり、「流」の字音で読むことを示す。二字熟語「鷦鷯」には「焦(L)「セウ」遼(L)「レウ」二音」とあり、「鷦」「鷯」それぞれの類音注を「二音」としてまとめて記載する。「音鴛之上声」のように声調まで示す特殊な例は「その他」に分類する。

## 仮名音注

仮名音注（Kana Glosses）は、片仮名表記による字音注である。漢字を用いて拗音・濁音を示す変則的な例も仮名音注に含める。

例えば、「正」には「和者ウ」とあり拗音を漢字で示し、「堂」には「俗云堕ウ」とあり濁音を漢字で示す。

## 声点

声点は、掲出字・音注字に付される声調符号（四声・六声）と清濁の別を示す。KRMでは、2025年3月の仕様変更後、ローマ字1文字で次のように示している。
