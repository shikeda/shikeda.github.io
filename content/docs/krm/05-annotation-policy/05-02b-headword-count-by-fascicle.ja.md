...
本ページは、[掲出字数の算出](../05-02-headword-count/)で示した算出方法にもとづき、名義抄120部それぞれの掲出項目数・掲出字数を集計した詳細資料である。算出方法そのものについては同ページを参照。
...
その後の点検（`krm_headword_chars.tsv`との突き合わせ）で、次の2件の誤りが見つかった。

- `kazama_location`: K02051221, `hanzi_entry`: 咥 — 直前の項目（K02051210「𠰍」）の`original_entry`欄に残っていた旧稿「ー（𠰍）／〻（𠰍）／〇」の名残で、構成字が誤って3字（𠰍・𠰍・咥）のまま登録されていた。`krm_notes.tsv`の当該箇所は「「ー〻」は元来前項の注なるべし」として既に見出し字数から除外しており、構成字も「咥」の1字に修正した。
- `kazama_location`: K02060310, `hanzi_entry`: 嘻／［囉］ — 脱字「囉」が構成字として未登録だったため補った。

これにより口部の内訳は次のように修正される（項目数は1,035のまま）。

| No  | 部 | 1字   | 2字   | 3字   | 4字 | 5字 | 6字以上 | 項目数 | 字数 |
|-----|---|------:|------:|-----:|----:|----:|------:|------:|---:|
| 014 | 口 | 858  | 147  | 16  | 9  | 4  | 1  | 1,035  | 1,273  |
...
さらにその後の点検で、次の項目に誤りが見つかった。

- `kazama_location`: K03117830, `hanzi_entry`: 櫫 — 見出しは「櫫」の1字だが、原文「豬木」のように2字にも見える箇所で、構成字に誤って「木」の1字が付属していた。1字に修正した。

これにより木部の内訳は次のように修正される（項目数は1,334のまま）。

| No  | 部 | 1字   | 2字   | 3字   | 4字 | 5字 | 6字以上 | 項目数 | 字数 |
|-----|---|------:|------:|-----:|----:|----:|------:|------:|---:|
| 029 | 木 | 1,045  | 244  | 40  | 4  | 0  | 1  | 1,334  | 1,675  |
...
その後の点検で、水部の構成字に次の2件の誤りが見つかった（項目数への影響はない）。

- `kazama_location`: K05003131, `hanzi_entry`: ー（洞）／逹 — 代用符号「ー」が指す「洞」（前項の掲出字）が構成字として登録されておらず、「逹」のみの1字になっていた。「洞」を補い2字とした。
- `kazama_location`: K05039810→K05039811, `hanzi_entry`: 風／［流］ — 構成字が「？風」という不審な1字（切り出し位置の誤りとみられる）で登録されていた。掲出字の位置を1コマ後ろに訂正し、「風」「流」（脱字）の2字とした。この修正に伴い、`krm_main.tsv`・`krm_notes.tsv`・`krm_wakun.tsv`の該当箇所の`kazama_location`／`tenri_location`も修正した。

これにより水部の内訳は次のように修正される（項目数は1,321のまま）。

| No  | 部 | 1字   | 2字   | 3字   | 4字 | 5字 | 6字以上 | 項目数 | 字数 |
|-----|---|------:|------:|-----:|----:|----:|------:|------:|---:|
| 041 | 水 | 1,136  | 150  | 27  | 4  | 0  | 4  | 1,321  | 1,565  |
...
上記の「桃／花／［石］」（K06012741）は、脱字「石」が構成字として未登録だったため、`krm_headword_chars.tsv`側を補った（項目数・字数に変更はない）。あわせて点検したところ、石部で構成字6字以上の項目は3件（K06004811・K06007621・K06011231）であり、上表の「6字以上」欄は4ではなく3が正しい。

| No  | 部 | 1字   | 2字   | 3字   | 4字 | 5字 | 6字以上 | 項目数 | 字数 |
|-----|---|------:|------:|-----:|----:|----:|------:|------:|---:|
| 051 | 石 | 307  | 56   | 6   | 3  | 0  | 3  | 375  | 472    |
...
酒井（1967）との項目数の相違とは別に、その後の点検で次の項目が見つかった。

- `kazama_location`: K06100631, `hanzi_entry`: 喜／憘／⿱⿱二口⿱廾心 — 前項 K06100620「憙」の義注「喜「注也」」が大字に誤って書写され、次項の掲出字であるかのように見えていた。左に朱筆で「注也」とあることから注文と判断し、掲出字から「喜」を除いて「憘／⿱⿱二口⿱廾心」の2字に修正した（`krm_main.tsv`・`krm_notes.tsv`とも修正）。

これにより心部の内訳は次のように修正される（項目数は908のまま）。

| No  | 部 | 1字   | 2字   | 3字   | 4字 | 5字 | 6字以上 | 項目数 | 字数 |
|-----|---|------:|------:|-----:|----:|----:|------:|------:|---:|
| 057 | 心 | 730  | 162  | 12  | 1  | 2  | 1  | 908  | 1,110  |
...
間に改行があり、ここで項目を二つに分けたとも考えられる。しかし、「饕」の異体字が連続する
...
**竹部**は酒井（1967）と池田ほか（2020）とで掲出項目数に差がないが、`krm_headword_chars.tsv`との突き合わせで次の項目の構成字に誤りが見つかった。

- `kazama_location`: K08079831, `hanzi_entry`: 𥲄／竹 — 構成字が「竹・淡・竹」の3字で登録されていたが、これは画像の切り出し位置の誤りによる。正しくは「𥲄・竹」の2字であり、修正した（1字目の画像参照は未確認のため空欄とした）。

これにより竹部の内訳は次のように修正される（項目数は492のまま）。
...
**弓部**の掲出項目数は、酒井（1967）が102項目、池田ほか（2020）が101項目で、1項目の差がある。
これは次の箇所が問題になると考えられる。
...
これ以外は、認定の仕方により数値が変動するもので、池田ほか（2020）のままとしていた。

その後、`krm_main.tsv`と、掲出字を構成字単位で記録した`krm_headword_chars.tsv`とを
`scripts/headword_count_by_radical.py`（krmリポジトリ）で突き合わせて再点検した結果、
14口部・29木部（さらなる修正）・41水部・51石部・57心部・82竹部・120雑部で、脱字の未反映や構成字の誤りなど、
追加の修正が必要であることが分かった（120雑部については「追記」を参照）。詳細は該当する巻の項に記した。15目部は酒井（1967）との
相違がもともと無かった部首だが、上記の点検にあわせて集計し直したところ、項目数は462となった。

[以上の点検](../05-02-headword-count/#掲出項目数の点検)、および上記の再点検結果を反映させて、
以下にすべての部首の掲出項目数を一覧する。掲出字数は明白な脱字を補った数値としている。
...
| 120 | 雑 | 1,074 | 333 | 73 | 14 | 4 | 3 | 1,501 | 2,058 |
|全体|
|   |  | 1字   | 2字   | 3字   | 4字 | 5字 | 6字以上 | 項目数 | 字数 |
| 合計 | | 24,681 | 6,674 | 975 | 165 | 50 | 62 | 32,607 | 42,338 |
...
2026年9月　`krm_main.tsv`の見出し（陟厘）は既に二字としていたが、`krm_headword_chars.tsv`の構成字が
合字「⿱陟厘」の1字のまま同期されていなかった。「陟」「厘」の2字に修正し、上記「まとめ」の雑部・合計の
数値に反映済み。
