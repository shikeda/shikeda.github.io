...
本章はProject Records（進捗記録）の一部であり、KRMデータベースの構築過程を記録するものである。規範的なCore Documentationとは性格が異なる。
...
## KRM Documentationの形成

リポジトリの履歴は2022年1月6日のコミットに始まる。同年1月、KRMに関する覚え書きと解説ページ群としてDocumentationのリポジトリ管理が始まった。2022年から2025年にかけて、資料概要、データ構造、入力手順、注釈方法、進捗記録、研究事例（case study）に関する記述を順次蓄積していった。2026年には、サイトの自動デプロイや外部データとの連携を支えるための整備を行った。2026年7月からは、蓄積してきた資料を明示的にKRM Documentation Projectとして再編し、Project Charter、Documentation Blueprint、Roadmap、編集方針、スタイルガイド、レビュー手順を導入した。この再編を経て、2026年8月15日に英語版の最初の完成ドラフト（Version 0.9）に至った。
