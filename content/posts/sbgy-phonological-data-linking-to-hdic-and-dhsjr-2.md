...
`gy_dhsjr_link.py`、`廣韻.csv`、`30-048-02_RMK.tsv`を用意して次を実行します。（python3で動作しない場合はpythonで実行してください）
...
ここで注意されるのは、CJK統合漢字が1,411 件あり、その中に内（U+5185）が見える点です。つまり`廣韻.csv`は「內（U+5167）」を見出しとしているため、マッチしなかったわけです。
...
今回、修正した観智院本類聚名義抄のデータ（30-048-02_RMK.tsv、27,985行）に適用した結果の概略は次のとおりとなります。ここではNIHUのTSVファイルに自作のJSONファイル二つを適用した結果を示します。未収録は6.3%ほど減らすことに成功しました。
